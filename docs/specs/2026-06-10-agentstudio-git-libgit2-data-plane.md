...
- A clean downstream SwiftPM package can consume the local development path, and release HTTPS/checksum manifest mode is evaluated. Actual distributable artifact download proof requires a hosted `CLibGit2Local.xcframework.zip` and `scripts/verify-hosted-libgit2-artifact.sh`.
